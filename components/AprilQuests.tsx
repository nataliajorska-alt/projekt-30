...
import { getAprilQuestsForDate, getOverdueAprilQuests, APRIL_NAME } from '@/lib/aprilData'
import { getPillar } from '@/lib/pillars'
import { todayKey } from '@/lib/gameLogic'
import { Check, Clock, SkipForward } from 'lucide-react'
import clsx from 'clsx'

export default function AprilQuests() {
      ...
      <div className="px-5 pt-5 pb-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h2 className="font-serif text-dark text-lg">Questy miesiąca</h2>
          </div>
          ...
          return (
            <div key={quest.id} className="rounded-xl border border-amber-200 bg-amber-50/60 p-4">
              <div className="flex items-start gap-2 mb-2">
                <div className="flex items-center gap-1.5 bg-amber-100 border border-amber-200 px-2 py-0.5 rounded-full flex-shrink-0 mt-0.5">
                  <Clock size={10} className="text-amber-600" strokeWidth={2} />
                  <span className="font-sans text-[10px] text-amber-700 font-medium uppercase tracking-wide">zaległe</span>
                </div>
                <div className="flex-1">
                  <p className="font-sans text-[10px] uppercase tracking-widest mb-0.5" style={{ color: pillar.color }}>
                    {pillar.icon} {pillar.shortName} · {quest.date.slice(5).replace('-', '.')}
                  </p>
                  <h3 className="font-serif text-dark text-sm">{quest.title}</h3>
                </div>
                <span className="font-sans text-xs text-muted-light flex-shrink-0">+{quest.xp} XP</span>
              </div>
              ...
              <div className="flex gap-2">
                ...
                >
                  <Check size={12} strokeWidth={2} /> Zrobione
                </button>
                <button
                  ...
                  title="Pomiń — nie będzie się pojawiało"
                >
                  <SkipForward size={12} strokeWidth={1.5} /> Pomiń
                </button>
              </div>
            </div>
          )
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3 mb-2">
                <div className="flex-1">
                  <p className="font-sans text-[10px] uppercase tracking-widest mb-1" style={{ color: pillar.color }}>
